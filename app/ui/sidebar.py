"""Minecraft-style sidebar navigation component - Modernized Edition

Enhanced with better visual hierarchy, modern touches, and polished aesthetics
...
class Sidebar(ft.Container):
    """Left navigation sidebar with modern Minecraft aesthetic
    ...
    def __init__(
        ...
        col.controls.append(
            ft.Container(
                ...
                padding=ft.Padding(left=12, right=12, top=14, bottom=12),
                expand=True,
            )
        )
    ...
    def _build_tab_button(self, tab: Dict[str, Any]) -> ft.Container:
        """构建单个标签按钮 with modern hover effects

        Args:
            tab: Tab definition dictionary

        Returns:
            ft.Container: Tab button container
        """
        selected = tab["id"] == self._selected_id
        icon_name = tab.get("icon", IconSet.GRID)
        label_text = tab.get("label", tab["id"])

        # Icon container with Minecraft block style
    ...
    def _handle_hover(self, e: ft.ControlEvent, tab_id: str) -> None:
        """Handle hover event for tab buttons with glow effect
        ...
        try:
            ...
            if e.data == "true":
                # Hover state - add glow and brighten
                if not is_selected:
    ...
    def set_current_save_name(
        ...
        Args:
            name: Save name to display
            path: Save path for tooltip
        """
        try:
            if name:
    ...
    def _apply_style(self, container: ft.Container, selected: bool) -> None:
        ...
        Args:
            container: Tab button container
        ...
        if isinstance(row, ft.Row) and len(row.controls) >= 2:
            ...
            if isinstance(icon_slot, ft.Container):
                ...
                if icon_slot.content and isinstance(icon_slot.content, ft.Icon):
                    icon_slot.content.color = THEME.mc_obsidian if selected else THEME.text_secondary
            if isinstance(
    ...
    def reorder_tabs(self, new_order: List[str]) -> None:
        ...
        Args:
            new_order: List of tab IDs in new order
        """
    ...
    def get_tab_order(self) -> List[str]:
        ...
        Returns:
            List[str]: List of tab IDs in current order
        """
