...
class HangDetector:
    ...
    def enable(self) -> None:
        ...
        self._thread = threading.Thread(target=self._detection_loop, daemon=True, name="HangDetector")
        self._thread.start()
    ...
    def _check_ui_hang(self, now: float) -> None:
        ...
        if elapsed >= self._ui_hang_threshold:
            if not self._ui_hang_alerted:
                ...
                self._log_warning(f"UI 可能已卡死：{elapsed:.0f}s 无心跳响应（阈值 {self._ui_hang_threshold:.0f}s）")
            ...
            for thread_id, frame in frames.items():
                ...
                if thread_id not in self._thread_snapshots:
                    ...
                    if snapshot["location"] != location:
                        ...
                        threshold = self._thread_block_threshold * 2 if is_benign else self._thread_block_threshold
                        ...
                        if elapsed >= threshold and not snapshot["alerted"]:
                            ...
                            self._log_warning(
                                f"线程 [{thread_name}] 可能阻塞：{elapsed:.0f}s 无进展（阈值 {threshold:.0f}s）| 堆栈: {stack_trace}"
                            )
