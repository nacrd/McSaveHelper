...
class NbtTreeEditor:
    ...
    def open_edit_dialog(self, path: str, value: Any, type_name: str) -> None:
        ...
        dialog.actions = [
            ft.TextButton(
                "暂存",
                on_click=lambda e: self._stage_edit(
                    path,
                    value,
                    type_name,
                    value_field,
    ...
    def open_add_field_dialog(self, parent_path: str, is_list: bool) -> None:
        ...
        key_field, type_dropdown, value_field, error_text, dialog = controls
        dialog.actions = [
            ft.TextButton(
                "新增",
    ...
    def _add_dialog_controls(self, parent_path: str, is_list: bool) -> tuple:
        ...
        type_dropdown = ft.Dropdown(
            label="类型",
            options=[ft.dropdown.Option(value) for value in FIELD_TYPE_OPTIONS],
            value="String",
            border_color=THEME.border_standard,
            text_size=13,
        )
        ...
        error_text = ft.Text("", size=12, color=THEME.error)
        target_type = "列表项" if is_list else "字段"
        dialog = ft.AlertDialog(
            title=ft.Text(
                f"新增 {target_type} 到 {parent_path}",
                color=THEME.text_primary,
            ),
            content=ft.Column(
                [key_field, type_dropdown, value_field, error_text],
                tight=True,
                spacing=8,
            ),
            actions=[],
    ...
    def _insert_value(
        self,
        parent_path: str,
        parent_parts: List[Union[str, int]],
        parent_node: Any,
        key: str,
        new_value: Any,
        is_list: bool,
        ...
        if is_list:
            ...
            self._notify(
                parent_parts + [old_length],
                None,
                new_value,
                f"{parent_path}[{old_length}]",
            )
            return
