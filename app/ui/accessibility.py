...
def ensure_accessible_text(
    ...
    """
    # 判断是否为大文本
    large_text = (size and size >= 18) or (weight and weight in [
        ft.FontWeight.BOLD, ft.FontWeight.W_700, ft.FontWeight.W_800, ft.FontWeight.W_900
    ] and size and size >= 14)
    
    passes, ratio = check_contrast_wcag(color, bgcolor, "AA", large_text or False)
    
    # 如果对比度不足，发出警告（仅开发时）
    if not passes:
        print(f"[A11Y WARNING] 对比度不足: {ratio:.2f}:1 (前景: {color}, 背景: {bgcolor})")
...
class KeyboardNavigable:
    ...
    def __init__(self, on_enter: Optional[Callable] = None, on_escape: Optional[Callable] = None):
...
def focus_trap(content: ft.Control, on_escape: Optional[Callable] = None) -> ft.Container:
    """创建焦点陷阱容器（用于模态对话框）
...
def announce_to_screen_reader(page: ft.Page, message: str, priority: str = "polite") -> None:
    """向屏幕阅读器宣告消息
