"""映射页面后台操作的最新任务协调器（Qt 版）。

复制自 Flet 树 ``app/ui/views/mappings_operations.py``（纯逻辑、无 Flet 依赖）；
Flet 树删除后本文件为唯一权威。
"""
...
class DebouncedLatestSave:
    """串行保存最新内存状态，并在关闭时刷新防抖窗口。"""

    def __init__(
        self,
        operations: LatestOperationGroup,
        save: Callable[[], None],
    ) -> None:
