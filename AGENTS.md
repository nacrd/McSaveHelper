...
- 不得破坏 `UI -> controllers -> services -> core` 的依赖方向。
...
- 一个函数完成一个可命名的动作；名称和返回值应概括其完整效果。
- 优先用 guard clause 和早返回处理错误、空值和不支持状态，减少主路径缩进。
...
- 所有公共函数、方法、回调和 dataclass 字段必须有类型注解。
...
- `except Exception` 仅用于进程/线程入口、事务回滚和 best-effort 清理，并必须保持失败语义。
- 禁止空 `except`。允许忽略的清理错误需说明原因，且不得覆盖主异常。
...
- 公共 API、复杂事务、格式算法和不直观的回调协议使用简洁 docstring。
