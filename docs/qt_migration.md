...
- 阶段 0 基础层已完成，`python main.py --qt` 可启动 Qt 应用。
...
- 阶段 2 已完成 `migrator`，继续复用框架中立的 `MigrationController`；
  `explorer` 已完成 Qt 壳层、“存档信息”、只读玩家列表/摘要、世界统计与
  实体/方块/容器搜索，以及 NBT/JSON 文件树、暂存审阅与安全提交；玩家编辑、
  区块 NBT 和地图尚未迁移。
- Flet 仍是默认后端，Qt 树不得导入 `app.ui` 或 `flet`。
...
```text
main.py (--qt) -> app/qtui/application.py (Qt 组合根, QMainWindow)
                        |
                        v
...
`main.py` 支持 `--qt` 参数选择 Qt 后端；缺省仍为 Flet，保证迁移期间应用可用、
全部既有测试通过。`--console` 行为不变。

```bash
python main.py          # Flet 后端（现状）
python main.py --qt     # Qt 后端（迁移中）
```
...
3. **阶段 2 —— 复杂视图（进行中）**
   ...
   `explorer` 壳层、存档信息、只读玩家列表/摘要、stats_tab 和搜索已完成；
   NBT/JSON 文件树、叶子值暂存审阅与事务提交已完成；待迁移玩家编辑、
   region_tab、区块 NBT 与地图交互。

4. **阶段 3 —— 地图画布**
   `mca_map_view` 的 `ft.GestureDetector` + `ft.Stack` + 形状层改为
   `QGraphicsView`/`QWidget.paintEvent`；`map_shapes` 逻辑（命中测试、坐标换算）
   保留为纯函数复用。

5. **阶段 4 —— 收尾**
   - 删除 `app/ui/`、Flet 管理器与 `flet`/`flet-desktop` 依赖。
   - `main.py` 去掉后端开关，Qt 成为唯一后端。
   - 删除本批复制的色板数据（`app/qtui/theme.py` 保留，删 `app/ui/theme.py`）。
   - 更新 Nuitka 构建与文档。
