...
from app.ui.views.explorer.explorer_view import (
    ExplorerView,
    map_world_index_frame_progress,
)
...
def test_index_frame_progress_is_monotonic_across_phases() -> None:
    validating = WorldIndexProgressFrame(
        Path("world"),
    ...
    discovering = WorldIndexProgressFrame(
        Path("world"),
    ...
    probing = WorldIndexProgressFrame(
        Path("world"),
    ...
    finalizing = WorldIndexProgressFrame(
        Path("world"),
    ...
    values = [
        map_world_index_frame_progress(frame)[0]
        for frame in (validating, discovering, probing, finalizing)
    ]
...
def test_discovering_progress_grows_with_file_count() -> None:
    small = WorldIndexProgressFrame(
        Path("world"),
        WorldIndexBuildPhase.DISCOVERING,
        1,
        None,
    ...
    small_value, small_stage = map_world_index_frame_progress(small)
    large_value, large_stage = map_world_index_frame_progress(large)

    assert small_stage == "discovering"
    assert large_stage == "discovering"
    assert 0.2 <= small_value < large_value < 0.6


def test_probing_progress_uses_completed_ratio() -> None:
    ...
    assert stage == "probing"
    assert value == 0.6 + 0.32 * 0.5


def test_complete_frame_maps_to_final_value() -> None:
    frame = WorldIndexProgressFrame(
        Path("world"),
...
def test_apply_load_progress_updates_bottom_bar_and_label() -> None:
    view = _bare_explorer(disposed=False, generation=1)
    updates: list[tuple[str, float]] = []
    view.app = cast(
        Any,
        SimpleNamespace(
            update_progress_with_task=lambda name, value: updates.append(
                (name, value)
            ),
        ),
    )
    view._world_label = cast(
        Any,
        SimpleNamespace(value="", color=None, update=lambda: None),
    )

    view._apply_load_progress(0.5, "正在加载存档...", 1)

    assert updates == [("正在加载存档...", 0.5)]
    assert view._world_label.value == "⏳ 正在加载存档... · 50%"


def test_apply_load_progress_ignores_stale_generation() -> None:
    view = _bare_explorer(disposed=False, generation=1)
    updates: list[tuple[str, float]] = []
    view.app = cast(
        Any,
    ...
    view._apply_load_progress(0.5, "正在加载存档...", 2)

    assert updates == []


def test_apply_index_progress_formats_stage_label() -> None:
    view = _bare_explorer(disposed=False, generation=1)
    ...
    view._apply_load_progress = cast(
        Any,
        lambda value, label, generation: applied.append((value, label)),
    )
    frame = WorldIndexProgressFrame(
        Path("world"),
        WorldIndexBuildPhase.DISCOVERING,
    ...
    view._apply_index_progress(frame, 1)

    expected_value = 0.2 + 0.4 * (1 - math.exp(-1234 / 800))
    assert applied == [
        (pytest.approx(expected_value), "正在发现文件 · 1234 个"),
    ]


def test_apply_index_progress_skips_complete_frame() -> None:
    view = _bare_explorer(disposed=False, generation=1)
    applied: list[tuple[float, str]] = []
    view._apply_load_progress = cast(
        Any,
...
def test_world_load_detaches_old_session_before_background_open() -> None:
    ...
    view.app = cast(
        Any,
        SimpleNamespace(
            show_progress=lambda _task_name: None,
            translate=lambda _key, default="": default,
    ...
    assert view._loaded_world_path == Path("new-world").resolve()


def test_same_world_selected_again_skips_reload() -> None:
    view = _bare_explorer(disposed=False, generation=4)
    loaded: list[Any] = []
    view._load_world = cast(Any, lambda path: loaded.append(path))
    view._loaded_world_path = Path("world-a").resolve()

    view.on_save_selected(str(Path("world-a")))

    assert loaded == []


def test_new_world_selection_still_reloads() -> None:
    view = _bare_explorer(disposed=False, generation=4)
    loaded: list[Any] = []
    view._load_world = cast(Any, lambda path: loaded.append(path))
    view._loaded_world_path = Path("world-a").resolve()

    view.on_save_selected(str(Path("world-b")))

    assert loaded == [str(Path("world-b"))]


def test_same_world_skip_ignores_none_current_world() -> None:
    view = _bare_explorer(disposed=False, generation=4)
    loaded: list[Any] = []
    view._load_world = cast(Any, lambda path: loaded.append(path))
    view._loaded_world_path = None

    view.on_save_selected(str(Path("world-a")))

    assert loaded == [str(Path("world-a"))]
...
def test_clear_save_cancels_world_tasks_and_resets_projection() -> None:
    ...
    view._task_scope = cast(Any, scope)
    view._loaded_world_path = Path("world").resolve()
    view._world_label = cast(
        Any,
        SimpleNamespace(value="old", color=None, update=lambda: None),
    )
...
def test_world_worker_publishes_shell_then_opens_same_read_context() -> None:
    ...
    assert len(opened) == 1
    assert [entry[0].event for entry in posted] == [
        "progress",
        "shell",
        "progress",
        "progress",
